...
class DumbReturnHandler:
    ...
    AOTAutograd may also generate graph with duplicated return item. E.g. https://gist.github.com/shunting314/e60df8ac21fbe2494337c10d02bd78dc
    ...
    def __init__(self, trace_inputs, trace_outputs, trace_inputs_inplace_update_bool):
        ...
        if debug:
            print(f"Number of duplicated outputs {len(self.trace_outputs) - len(self.deduped_trace_outputs)})")
        ...
        if debug:
            print(f"Number trace input {len(trace_inputs)}, number trace output {len(trace_outputs)}")
